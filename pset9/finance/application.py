...
def index():
    ...
    rows = db.execute("""SELECT symbol, SUM(shares) FROM transactions
...
def buy():
    if request.method == "GET":
        ...
        db.execute("INSERT INTO transactions VALUES (:user_id, :symbol, :shares, :date_time, :price, :total)",
...
def sell():
    if request.method == "GET":
        ...
        rows = db.execute("""SELECT SUM(shares) FROM transactions
                                JOIN users ON id == user_id
                                WHERE user_id == :user_id
                                AND symbol == :symbol
        ...
        db.execute("INSERT INTO transactions VALUES (:user_id, :symbol, :shares, :date_time, :unit_price, :total_price)",
                   user_id=session["user_id"], symbol=symbol,
